...
#include <wb_controller/LoggerNumeric.h>
#include <Eigen/Core>
#include <type_traits>
...
inline void publish_imp(RealTimePublisher<data_t>* obj, const ros::Time& time)
{
    if(obj->getPubPtr()->trylock() && obj->getDataPtr())
    {
        ...
        for(unsigned int i = 0; i < size; i++)
                obj->getPubPtr()->msg_.data.data[i] = static_cast<float>(obj->getDataPtr()->operator[](i));
        obj->getPubPtr()->msg_.time.data = time;
        obj->getPubPtr()->unlockAndPublish();
    }
}
...
template <typename data_t>
class RealTimePublisher : public RealTimePublisherBase<data_t,wb_controller::LoggerNumeric>
{
public:

    RealTimePublisher(const ros::NodeHandle& ros_nh, const std::string topic_name, data_t* const data)
        :RealTimePublisherBase<data_t,wb_controller::LoggerNumeric>(ros_nh,topic_name,data)
    {
