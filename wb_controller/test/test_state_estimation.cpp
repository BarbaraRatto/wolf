...
void update(const sensor_msgs::JointState::ConstPtr& msg)
{
    ...
    _robot->getModelImp()->setJointVelocity(_joint_velocities);
    _robot->getModelImp()->setJointPosition(_joint_positions);
...
int main(int argc, char **argv)
{
    ...
    _qp_estimation.reset(new OpenSoT::floating_base_estimation::qp_estimation(_robot->getModelImp(),_robot->getFootNames(),contact_matrix));
